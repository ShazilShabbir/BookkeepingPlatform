...
function csvDownload(data: ChartData[]) {
  const header = 'Month,Expenses\n';
  const rows = data.map(r => `${r.month},${r.expenses}`).join('\n');
  const blob = new Blob([header + rows], { type: 'text/csv' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = 'expense-trend.csv'; a.click();
  URL.revokeObjectURL(url);
}

export default memo(function ExpenseChart({ userId, months = 12, onMonthClick }: { userId: string; months?: number; onMonthClick?: (month: string) => void }) {
  ...
  return (
    <Card padding="md">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold text-surface-900">Expenses Trend ({months} Months)</h3>
        {data.length > 0 && (

  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'mark-paid') {
        ...
        if (invoice.status === 'cancelled') return res.status(400).json({ error: 'Cannot mark a cancelled invoice as paid' });
        invoice.status = 'paid';
        invoice.paidAt = new Date();
        await invoice.save();
        await logAction({ userId: uid, action: 'update', resource: 'invoice', resourceId: id as string, details: { invoiceNumber: invoice.invoiceNumber, markPaid: true }, req });
    ...
    console.error('[invoices/[id]] error:', e?.message);

...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.error('[invoices/send] error:', e?.message);
  ...
  return `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="padding: 32px; background: #fff;">
        ...
        <p style="color: #6b7280; font-size: 15px;">You have a new invoice from BookKeep for <strong>${sym}${invoice.total.toFixed(2)}</strong>, due on <strong>${dueStr}</strong>.</p>
        <table style="width: 100%; border-collapse: collapse; margin: 24px 0;">
          ...
          ${invoice.lineItems.map((li: any) => `
            <tr style="border-bottom: 1px solid #f3f4f6;">
              ...
              <td style="padding: 10px 12px; font-size: 14px; color: #374151; text-align: right;">${sym}${li.amount.toFixed(2)}</td>
            </tr>
          `).join('')}
        </table>
        <div style="border-top: 2px solid #f3f4f6; padding-top: 16px; text-align: right;">
          ${invoice.taxRate > 0 ? `<p style="margin: 4px 0; color: #6b7280;">Subtotal: ${sym}${invoice.subtotal.toFixed(2)}</p><p style="margin: 4px 0; color: #6b7280;">Tax (${invoice.taxRate}%): ${sym}${invoice.taxAmount.toFixed(2)}</p>` : ''}
          <p style="margin: 4px 0; font-size: 20px; font-weight: 700; color: #111827;">Total: ${sym}${invoice.total.toFixed(2)}</p>
        </div>
      </div>
